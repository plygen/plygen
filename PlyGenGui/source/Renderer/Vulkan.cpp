...
namespace plygen::gui::renderer
{
    ...
    VkPhysicalDevice RendererVulkan::SetupVulkan_SelectPhysicalDevice()
    {
        uint32_t gpu_count;
    ...
    void RendererVulkan::SetupVulkan(std::vector<const char*> instance_extensions)
    {
        VkResult err;
    ...
    void RendererVulkan::SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
    {
        wd->Surface = surface;
    ...
    void RendererVulkan::FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
    {
        VkResult err;
    ...
    void RendererVulkan::FramePresent(ImGui_ImplVulkanH_Window* wd)
    {
    ...
    void RendererVulkan::CleanupVulkanWindow()
    {
        ImGui_ImplVulkanH_DestroyWindow(m_VkInstance, m_VkDevice, &m_WindowData, m_VkAllocator);
    }

    RendererVulkan::RendererVulkan(unsigned Width, unsigned Height, std::string Title)
        : RendererBase(), m_Width(Width), m_Height(Height), m_Title(Title), m_SwapChainRebuild(false), m_VkAllocator(nullptr), m_VkInstance(VK_NULL_HANDLE), m_VkPhysicalDevice(VK_NULL_HANDLE),
    ...
    bool RendererVulkan::Initialize()
    {
        ...
        {
            printf("[RendererVulkan::Initialize] glfwVulkanSupported() returned false!\n");
            return false;
        }
        ...
        m_OriginalDropFunction = glfwSetDropCallback(m_GlfwWindow,
                                                     ...
                                                     {
                                                         RendererVulkan* vulkanImplementation = reinterpret_cast<RendererVulkan*>(glfwGetWindowUserPointer(Window));
    ...
    bool RendererVulkan::ShallRender()
    {
